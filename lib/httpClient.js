  ...
  _request: function(method, path, data, auth, callback){
    ...
    function makeRequest(){
      ...
      if(self._root._options.debug){
        // Already serialized - parse is a great way to make a deep clone
        var redactedData = JSON.parse(requestData);
        var redactedFields = self._root._options.redacted || [];
        for (var i = 0; i < redactedFields.length; ++i) {
          var field = redactedFields[i];
          if (redactedData[field] !== undefined) {
            redactedData[field] = '[REDACTED]';
          }
        }
        redactedData = JSON.stringify(redactedData);
        var curl = "curl"
        curl += " -X " + method
